...
// Self-hosted fonts (generated by `npm run fonts`, committed under fonts/).
// We inline the @font-face CSS into the page <head> and preload the
// above-the-fold faces, so there's no third-party origin and no FOUT.
const FONTS_DIR = path.join(__dirname, "fonts");
const readFont = (f) => {
  try {
    return fs.readFileSync(path.join(FONTS_DIR, f), "utf8");
  } catch {
    return null;
  }
};
const fontFaceCss = readFont("fonts.css") || "";
const fontPreload = JSON.parse(readFont("manifest.json") || "[]");
...
module.exports = function (eleventyConfig) {
  ...
  // Serve the self-hosted fonts at /fonts/**.
  ...
  // Inlined @font-face CSS + the list of faces to preload (see top of file).
